...
public class F_CLIENTES extends javax.swing.JFrame {
    ...
    String secao, rf, nome, sNomeSecao, obs, tipo, departamento, setartipo,tiposetado,nomeDigitado, nomeSecaoIni, nomeClienteIni = "";
    boolean cadastrando;  //controla no botão gravar entre gravar novo registro e gravar alteração de um registro
    String sqlDefault = "select c.*, s.nome as secao,s.ramal from tblclientes c, tblsecoes s where s.codigo = c.secaoid and c.status='ATIVO' AND c.tipo='F' order by c.nome,s.nome";
    ...
    private void jTabelaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTabelaMouseClicked
        ...
        nomeClienteIni  = txtNOME.getText();
        idUsuario     = Integer.parseInt(txtCODIGO.getText());
        umMetodo.EUsuario(txtNOME.getText());
    ...
    private void filtrarPorDigitacao(String pPesq) {
        ...
        PreencherTabela("select c.*, s.nome as secao,s.ramal from tblclientes c, tblsecoes s where (c.nome like '%" + pPesq + "%'" + " "
                ...
                + "and c.status='ATIVO' and c.tipo='F' order by c.nome, s.nome");
    ...
    private void editarRegistro() 
    {
        ...
        int idSecao = umMetodo.retornaCodigo("tblsecoes", "nome", cmbSecao.getSelectedItem().toString());           
        ...
        //JOptionPane.showMessageDialog(rootPane, "NOME DO CLIENTE INICIALMENTE "+nomeClienteIni);
                      
        //ATUALIZAR TB O CLIENTE DA TABELA DE USUARIOS SE O MESMO TB FOR USUARIO DO SISTEMA
        int codigoUsuario = umMetodo.getCodigoPassandoString("tblusuarios", "nome", nomeClienteIni);        
        
        umMetodo.alterarRFUsuarioSeForAlteradoRFColaborador(rf, codigoUsuario);
        
        if(!nome.equals(nomeClienteIni)){
            
            if(isUsuario){
                umMetodo.alterarNomeUsuarioSeForAlteradoNomeColaborador(nome,codigoUsuario);
            }
            
        }                              
        
        //SE STATUS = INATIVO ATUALIZAR O CLIENTEID PARA (SEM USUARIO DA SEÇÃO) EM DE TODOS OS EQUIPAMENTOS DESSE COLABORADOR => UPDATE TBLPATRIMONIOS SET CLIENTEID=? WHERE CLIENTEID = IDDESSECLIENTE
        ...
        //SE ALTERAR A SEÇÃO DO USUARIO TRANSFERIR JUNTAMENTE COM SEUS EQUIPTOS PARA O NOVO SETOR SE ASSIM O DESEJAR 
        if(!cmbSecao.getSelectedItem().toString().equals(nomeSecaoIni))
        {            
            ...
            if(umabiblio.ConfirmouOperacao("Deseja transferir os equipamentos juntamente com o usuário?", "Transferindo usuário de setor!")){                
                //JOptionPane.showMessageDialog(rootPane, "ID SECAO : "+idSecao+"  CODIGO DO CLIENTE : "+txtCODIGO.getText());
                umMetodo.atualizarSecaoDosPatrimoniosDoUsuarioNaNovaSecao(idSecao, Integer.parseInt(txtCODIGO.getText()));  
            }else{
                //USUARIO TRANSFERIDO PARA OUTRO SETOR SEM OS EQUIPTOS QUE FICARAM NO SETOR ATUAL COM USUARIOID = (SEM USUARIO DO SETO ATUAL)        
                nomeSemUsuarioDaSecao = "SEM USUARIO "+nomeSecaoIni; 
                
                //SETA ID DO USUARIO COMO SENDO : SEM USUARIO DA SECAO
                idSemUsuarioDaSecao   = umMetodo.getCodigoPassandoString("tblclientes", "nome", nomeSemUsuarioDaSecao); // EX : 293 SEM USUARIO DOG                
                umMetodo.atualizarPatrimoniosParaSemUsuarioDaSecaoDoColaboradorTransferido(idSemUsuarioDaSecao,Integer.parseInt(txtCODIGO.getText()));                
            }
        }
